...
<%--
   ...
   / 는 바로 webapp 폴더가 된다.
   이렇게 포함시키면 index.jsp 페이지와 navbar.jsp 페이지가 공통으로 클라이언트에게 응답을 하게 된다.
   포함시킬 페이지에 파라미터를 전달할 수 있다
   thisPage 라는 파라미터명으로 index 라는 문자열을 전달하는 방법은 아래와 같다
   <jsp:param value="index" name="thisPage"/> => navbar.jsp?thisPage=index 와 같은 효과를 냄
 --%>
