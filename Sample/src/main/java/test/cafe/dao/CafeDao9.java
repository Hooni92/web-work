...
public class CafeDao9 {
	private static CafeDao9 dao;
	
	private CafeDao9() {}
	
	public static CafeDao9 getInstance() {
		if(dao==null) {
			dao=new CafeDao9();
		}
		return dao;
	...
	public boolean delete(int num) {
		...
		try {
			conn = new DbcpBean().getConn();
			//실행할 sql 문
			String sql = "DELETE FROM board_cafe9"
					+ " WHERE num=?";
			pstmt = conn.prepareStatement(sql);
			//? 에 값 바인딩하기
			pstmt.setInt(1, num);
	...
	public boolean insert(CafeDto dto) {
		...
		try {
			conn = new DbcpBean().getConn();
			//실행할 sql 문
			String sql = "INSERT INTO board_cafe9"
	...
	public int getCount() {
		...
		try {
			conn = new DbcpBean().getConn();
			//select 문의 뼈대 구성하기
			String sql = "SELECT MAX(ROWNUM) AS count FROM board_cafe9";
			pstmt = conn.prepareStatement(sql);
			//? 에 값 바인딩 할게 있으면 하기
