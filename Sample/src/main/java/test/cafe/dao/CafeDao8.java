...
public class CafeDao8 {
	private static CafeDao8 dao;
	
	private CafeDao8() {}
	
	public static CafeDao8 getInstance() {
		if(dao==null) {
			dao=new CafeDao8();
		}
		return dao;
	...
	public boolean delete(int num) {
		...
		try {
			conn = new DbcpBean().getConn();
			//실행할 sql 문
			String sql = "DELETE FROM board_cafe8"
					+ " WHERE num=?";
			pstmt = conn.prepareStatement(sql);
			//? 에 값 바인딩하기
			pstmt.setInt(1, num);
	...
	public boolean insert(CafeDto dto) {
		...
		try {
			conn = new DbcpBean().getConn();
			//실행할 sql 문
			String sql = "INSERT INTO board_cafe8"
	...
	public int getCount() {
		...
		try {
			conn = new DbcpBean().getConn();
			//select 문의 뼈대 구성하기
			String sql = "SELECT MAX(ROWNUM) AS count FROM board_cafe8";
			pstmt = conn.prepareStatement(sql);
			//? 에 값 바인딩 할게 있으면 하기
